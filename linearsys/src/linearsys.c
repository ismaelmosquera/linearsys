...
{
	int i;
	Matrix* _q = create_matrix(rows_matrix(qr_q(qr)), 1);
	for(i = 0; i < rows_matrix(_q); i++) _q->_data[i] = v->_data[i];
	return upper_system_solver(qr_r(qr), get_column_vector(mul_matrix(transpose_matrix(qr_q(qr)), _q), 0));
}

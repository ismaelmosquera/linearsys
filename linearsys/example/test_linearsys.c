...
r1 = rotation_matrix(1.0, 0.0, 0.0, PI / 4.0);
...
Matrix* pM = pseudoinverse_matrix(m);
print_matrix(pM);
printf("\n");
printf("Note that order(I) must be equal to min(m,n)\n");
printf("where m and n are rows(M) and columns(M) respectively, so\n");
printf("pM^-1 * M= I:\n");
print_matrix(mul_matrix(pM, m));
printf("\n");
printf("Let us try with a 2x3 matrix.\n");
printf("singular Value Decomposition\n");
m = load_matrix("m.dat");
printf("M:\n");
print_matrix(m);
